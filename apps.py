...
def predict():
    ...
    img = image.load_img(predict_image_path, target_size=(256, 256, 3))
    ...
    return render_template("classifications.html", img_path = predict_image_path, 
                        predictionmodelmax = class_names[np.argmax(prediction_array_model)],
                        predictionmodelmin = class_names[np.argmin(prediction_array_model)],
                        # predictionmodelmid = class_names[argmedian(prediction_array_model)],
                        # sorted_indexes = np.argsort(prediction_array_model),  
                        predictionmodelmid = class_names[median_index], 
                        confidencemax = '{:2.0f}%'.format(100 * np.max(prediction_array_model)),
                        confidencemedian = '{:2.0f}%'.format(100 * np.median(prediction_array_model)),
                        confidencemin = '{:2.0f}%'.format(100 * np.min(prediction_array_model))     
